...
//	key_code ret = 0;
//
//	unsigned char row_mask[shift_array.dev_count];
...
//	single_key_done: ;
	...
	//TODO FLASH STUFF
//	while(ret == 0){
//		for(unsigned char row=0;row<KEYBOARD_ROWS;row++){
//			//Set current column high so that rows can be read
//			row_mask = (1<<(7-row));
...
//{
//	key_code ret = 0;
//	static unsigned char shift_modifier = 0;
//	static unsigned char row_mask = 0x00;
//
...
//
//	if(shift_modifier)
//		return lookup_char[(unsigned char)ret].modified;
//	else
//		return lookup_char[(unsigned char)ret].unmodified;
//}
//
...
//
//	//debounce stuff
//	unsigned char button_input[KEYBOARD_ROWS][KEYBOARD_COLS] = {0};
//	unsigned char button_last_state[KEYBOARD_ROWS][KEYBOARD_COLS] = {0};
//	unsigned char button_cur_state[KEYBOARD_ROWS][KEYBOARD_COLS] = {0};
...
//										lookup_str = lookup_char[(unsigned char)input_char].modified;
//									else
//										lookup_str = lookup_char[(unsigned char)input_char].unmodified;
