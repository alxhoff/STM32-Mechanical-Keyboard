 ...
 */
static uint32_t dma_buffer[(2) * (LED_CFG_LEDS_PER_DMA_IRQ)
		* (LED_CFG_BYTES_PER_LED * 8)];
...
//        DBG_PIN_IRQ_LOW;
	...
	if (led_cycles_cnt < LED_RESET_PRE_MIN_LED_CYCLES) {
	...
	} else if (led_cycles_cnt
			< (LED_RESET_PRE_MIN_LED_CYCLES + LED_CFG_COUNT)) {
		/*
...
#if LED_CFG_LEDS_PER_DMA_IRQ == 1
		led_fill_led_pwm_data(next_led,
				&dma_buffer[tc * DMA_BUFF_ELE_HALF_LEN]);
#endif /* LED_CFG_LEDS_PER_DMA_IRQ == 1 */
#if LED_CFG_LEDS_PER_DMA_IRQ > 1
        uint32_t counter = 0;
        /*
        ...
        if (counter < LED_CFG_LEDS_PER_DMA_IRQ) {
			...
			< (LED_RESET_PRE_MIN_LED_CYCLES + LED_CFG_COUNT
					+ LED_RESET_POST_MIN_LED_CYCLES + LED_CFG_LEDS_PER_DMA_IRQ)) {
		/*
		...
		if (led_cycles_cnt
				< (LED_RESET_PRE_MIN_LED_CYCLES + LED_CFG_COUNT
						+ 2 * LED_CFG_LEDS_PER_DMA_IRQ)) {
			memset(&dma_buffer[tc * DMA_BUFF_ELE_HALF_LEN], 0x00,
					DMA_BUFF_ELE_HALF_SIZEOF);
		}
	} else {
		/*
...
void DMA1_Ch4_7_DMA2_Ch1_5_DMAMUX1_OVR_IRQHandler(void) {
	if (LL_DMA_IsEnabledIT_HT(DMA2, LL_DMA_CHANNEL_5)
			&& LL_DMA_IsActiveFlag_HT5(DMA2)) {
		LL_DMA_ClearFlag_HT5(DMA2);
		led_update_sequence(0);
	}
	if (LL_DMA_IsEnabledIT_TC(DMA2, LL_DMA_CHANNEL_5)
...
//    DBG_PIN_DATA/_FILL_HIGH;
	if (ledx < LED_CFG_COUNT) {
		uint32_t r, g, b;

		led_fill_counter_arr[led_fill_counter++] = ledx;

		r = (uint8_t) (((uint32_t) leds_color_data[ledx * LED_CFG_BYTES_PER_LED
				+ 0] * (uint32_t) brightness) / (uint32_t) 0xFF);
		g = (uint8_t) (((uint32_t) leds_color_data[ledx * LED_CFG_BYTES_PER_LED
				+ 1] * (uint32_t) brightness) / (uint32_t) 0xFF);
		b = (uint8_t) (((uint32_t) leds_color_data[ledx * LED_CFG_BYTES_PER_LED
				+ 2] * (uint32_t) brightness) / (uint32_t) 0xFF);
...
static uint8_t led_start_transfer(void) {
	...
	for (uint32_t i = 0, index = LED_RESET_PRE_MIN_LED_CYCLES;
			index < 2 * LED_CFG_LEDS_PER_DMA_IRQ; ++index, ++i) {
...
static void tim2_init(void) {
	...
	LL_DMA_SetDataTransferDirection(DMA2, LL_DMA_CHANNEL_5,
			LL_DMA_DIRECTION_MEMORY_TO_PERIPH);
	LL_DMA_SetChannelPriorityLevel(DMA2, LL_DMA_CHANNEL_5,
...
void SysTick_Handler(void) {
	...
	if (++counter % 10 == 0) {
		...
		if (fade_value == 0) {
			for (size_t i = 0; i < LED_CFG_COUNT; ++i) {
				leds_color_data[i * LED_CFG_BYTES_PER_LED + 0] =
						color_counter & 0x01 ? 0xFF : 0x00;
				leds_color_data[i * LED_CFG_BYTES_PER_LED + 1] =
						color_counter & 0x02 ? 0xFF : 0x00;
				leds_color_data[i * LED_CFG_BYTES_PER_LED + 2] =
						color_counter & 0x04 ? 0xFF : 0x00;
			}
			color_counter++;
		}

		/* Calculate new brightness */
		brightness = (uint8_t) (quad_calc((float) fade_value / (float) 0xFF)
				* (float) 0x3F);
